...
const SPLIT_TYPES: {
...
}[] = [
  {
    id: "equal",
    label: "Equal Split",
    description: "Split evenly among all",
...
interface Member {
  ...
  name: string;
  selected: boolean;
  customAmount?: string; // For custom split amounts
}
...
export default function AddExpenseScreen() {
  ...
  const updateCustomAmount = (memberId: string, amount: string) => {
    setMembers(
      members.map((m) =>
        m.id === memberId ? { ...m, customAmount: amount } : m,
      ),
    );
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedMembers.length > 0) {
        ...
        if (splitType === "custom") {
          // Custom split - use individual amounts
          await Promise.all(
            selectedMembers.map((member) => {
              const customAmt = parseFloat(member.customAmount || "0");
              return createExpenseShare({
                expenseId,
                userId: member.id,
                shareAmount: customAmt,
              });
            }),
          );
        } else {
          // Equal split
          shareAmount = expenseAmount / selectedMembers.length;
          await Promise.all(
            selectedMembers.map((member) =>
              createExpenseShare({
                expenseId,
  ...
  // Calculate per person amount or total of custom amounts
  const perPersonAmount =
    amount && selectedMembersCount > 0
      ? (parseFloat(amount) / selectedMembersCount).toFixed(2)
      : "0.00";

  const totalCustomAmount = members
    .filter((m) => m.selected)
    .reduce((sum, m) => sum + parseFloat(m.customAmount || "0"), 0);

  const customAmountError =
    splitType === "custom" &&
    amount &&
    totalCustomAmount !== parseFloat(amount);
        ...
        >
                      ...
                      styles.paidByOption,
                      {
                        backgroundColor:
                          paidBy === member.id
                            ...
                            : colors.card,
                        borderColor:
                          paidBy === member.id ? Colors.primary : colors.border,
                      },
                    ]}
                  ...
                  >
                    ...
                    <Text
                      style={[
                        ...
                        {
                          color:
                            paidBy === member.id ? Colors.primary : colors.text,
                        },
                      ]}
                    >
                      {member.name}
                  ...
                  styles.splitOption,
                  {
                    backgroundColor:
                      splitType === type.id
                        ...
                        : colors.card,
                    borderColor:
                      splitType === type.id ? Colors.primary : colors.border,
                  },
                ]}
                ...
                <View style={styles.splitContent}>
                  <Text
                    style={[
                      ...
                      {
                        color:
                          splitType === type.id ? Colors.primary : colors.text,
                      },
                    ]}
                  >
                    {type.label}
          ...
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              ...
              {splitType === "equal" && (
                <Text
                  style={[styles.perPersonLabel, { color: Colors.primary }]}
                >
                  {currency === "INR" ? "₹" : currency === "EUR" ? "€" : "$"}
                  {perPersonAmount} each
                </Text>
              )}
              {splitType === "custom" && customAmountError && (
                <Text style={[styles.perPersonLabel, { color: Colors.error }]}>
                  Total: {totalCustomAmount.toFixed(2)} / {amount}
                </Text>
              )}
            </View>
                  ...
                  {splitType === "custom" && member.selected && (
                    ...
                    >
                      ...
                      <TextInput
                        ...
                        onChangeText={(text) =>
                          updateCustomAmount(member.id, text)
                        }
                        keyboardType="decimal-pad"
                      />
                    </View>
