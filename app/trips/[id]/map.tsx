...
export default function MapScreen() {
  ...
  const filteredItems =
    selectedDay !== null
      ? items.filter(
          (item) =>
            item.startTime &&
            new Date(item.startTime).toDateString() === days[selectedDay],
        )
      : items;
  ...
  const handleOptimizeRoute = async () => {
    const validPoints = filteredItems.filter(
    ...
    if (validPoints.length < 2) {
      Alert.alert(
        "Route Optimization",
        "Need at least 2 locations to show a route.",
      );
      return;
    }
    ...
    try {
      ...
      if (points.length > 0) {
        ...
        Alert.alert(
          "Route Calculated",
          "Showing the driving route between your locations.",
        );
      } else {
        ...
        Alert.alert(
          "Route Info",
          "Could not fetch exact roads. Showing straight lines instead.",
        );
      }
    } catch (error) {
        ...
        {error && (
          <View style={styles.errorOverlay}>
            <EmptyState
              ...
              description={
                error.message || "There was an error loading map markers."
              }
            />
          </View>
        )}
          ...
          {days.map((day, index) => (
            <TouchableOpacity
              ...
              style={[
                ...
                {
                  borderColor:
                    selectedDay === index ? Colors.primary : colors.border,
                },
              ]}
              onPress={() => {
          ...
          >
            <View
              style={[
                ...
                {
                  backgroundColor:
                    getCategoryColor(selectedItem.category) + "15",
                },
              ]}
            >
              <Ionicons
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                router.push(`/trips/${id}/itinerary/${selectedItem.id}`)
              }
              activeOpacity={0.8}
            >
        ...
        <View style={styles.placesListContainer}>
          <View style={styles.listHeader}>
            <Text style={[styles.listTitle, { color: colors.textSecondary }]}>
              ...
              {filteredItems.length === 1 ? "Place" : "Places"}
            </Text>
            {selectedItem && (
              <TouchableOpacity onPress={() => setSelectedItem(null)}>
                <Text
                  style={[styles.clearSelection, { color: Colors.primary }]}
                >
                  Clear
                </Text>
              </TouchableOpacity>
